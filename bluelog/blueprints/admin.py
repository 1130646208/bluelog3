...
@admin_bp.route('/post/batch/delete', methods=['POST'])
@login_required
def delete_post_batch():
    ids = []
    if request.method == 'POST':
        ids = request.form.get('ids').split(',')
    if ids:
        for _id in ids:
            post = Post.query.get_or_404(int(_id))
            db.session.delete(post)
        db.session.commit()
    flash('{} Posts deleted.'.format(len(ids)), 'success')
...
@admin_bp.route('/comment/batch/delete', methods=['POST'])
@login_required
def delete_comment_batch():
    ids = []
    if request.method == 'POST':
        ids = request.form.get('ids').split(',')
    if ids:
        for _id in ids:
            comment = Comment.query.get_or_404(int(_id))
            db.session.delete(comment)
        db.session.commit()
    flash('{} Comments deleted.'.format(len(ids)), 'success')
    # flash('{} Comments deleted.', 'success')
    return redirect_back()
